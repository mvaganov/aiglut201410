...
void FSM_Pursuit::execute(Agent * a, int a_ms) {
	...
	a->acceleration = seek(pursue, a);
	//a->acceleration += obstacleAvoidance(&validObstacles, &a->sensorArea, a, 0) * 20;
	a->direction = (pursue - a->body.center).normal();
}

void FSM_Pursuit::draw(Agent * a, GLUTRenderingContext * g_screen) {
	for(int i = 0; i < validObstacles.size(); ++i) {
		validObstacles[i]->glDraw(true);
	}
	g_screen->setColor(0x0088ff);
	g_screen->drawCircle(pursue, a->body.radius, false);
